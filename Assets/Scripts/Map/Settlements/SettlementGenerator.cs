...
public static class SettlementGenerator
{
    ...
    public static Settlement GenerateSettlement(GameModel model, Region region)
    {
        ...
        while (Connectors.Count > 0)
        {
            ...
            if (rng < 0.6f)
            {
                Building b = AddBuilding(model, connectorPosition, connectorDirection);
    ...
    /// </summary>
    private static Building AddBuilding(GameModel model, Vector2Int source, Direction dir)
    {
        ...
        Building b = BuildingGenerator.GenerateBuilding(model.TilemapGenerator, settings);
